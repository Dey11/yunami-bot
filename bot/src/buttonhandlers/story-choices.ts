...
export const handler = {
  id: /^(?!start:)(?!engine:)(?!choice:)(?!select:)(?!role:)(?!createProfile)(?!register).+/,
  async execute(interaction: any) {
    ...
    if (!session) {
      await interaction.reply({
        content:
          'No active session for this button. Please start a new story or use valid controls.',
    ...
    for (const node of Object.values(storyData.nodes) as any[]) {
      ...
      const found = node.choices.find(
        (c: any) => c.id === interaction.customId
      );
      if (found) {
        matchedChoice = found;
        break;
    ...
    if (matchedChoice) {
      ...
      if (matchedChoice.nextNodeId) {
        ...
        if (nextNode.type) {
          ...
          const [embed, buttons, image] = await storySceneBuilder(
            matchedChoice.nextNodeId,
            storyData
          );
        ...
        await interaction.reply({
          content: 'The story ends here... for now.',
          flags: MessageFlags.Ephemeral,
        });
      }
      return;
      ...
      await interaction.reply({
        content:
          'This interaction belongs to an expired or different session. You have likely started a new story.',
        flags: MessageFlags.Ephemeral,
      });
    }
  },
